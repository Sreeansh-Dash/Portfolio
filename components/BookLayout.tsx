...
const BookLayout: React.FC<BookLayoutProps> = ({ 
  ...
  return (
    <div className="book-container w-full max-w-7xl mx-auto relative perspective-1500">
      ...
      <div 
        className={`
            bg-paper-light dark:bg-paper-dark w-full aspect-[1/1.5] md:aspect-[3/2] lg:aspect-[1.5/1] 
      ...
      >
        ...
        <div className="absolute inset-0 bg-paper-texture opacity-40 pointer-events-none mix-blend-multiply dark:mix-blend-soft-light z-0"></div>
        
        {/* Center Spine Shadow for 3D effect */}
        <div className="absolute left-1/2 top-0 bottom-0 w-16 -ml-8 spine-shadow-overlay z-20 pointer-events-none mix-blend-multiply dark:mix-blend-overlay hidden md:block"></div>
        
        {/* Dynamic Page Flip Shadow - Sweeps across during animation */}
        <div className="absolute inset-0 page-shadow z-30 pointer-events-none"></div>

        {/* Mobile: Top to Bottom separation line */}
        <div className="absolute top-1/2 left-0 right-0 h-px bg-gray-300 dark:bg-gray-700 md:hidden z-20"></div>

        {/* Navigation Controls - Unobtrusive until hovered */}
        {onPrev && (
            <button 
                onClick={onPrev}
                className="absolute bottom-4 left-4 md:bottom-8 md:left-8 z-40 group flex items-center gap-2 text-primary dark:text-white transition-all duration-300 outline-none"
            >
                ...
                <div className="w-10 h-10 flex items-center justify-center rounded-full bg-white/0 group-hover:bg-white/20 dark:group-hover:bg-black/20 transition-all duration-300 backdrop-blur-[2px]">
                    <span className="material-icons group-hover:-translate-x-1 transition-transform text-2xl opacity-30 group-hover:opacity-100">arrow_back</span>
                </div>
        ...
        {onNext && (
            <button 
                onClick={onNext}
                className="absolute bottom-4 right-4 md:bottom-8 md:right-8 z-40 group flex items-center gap-2 text-primary dark:text-white transition-all duration-300 text-right outline-none"
            >
                {/* Text - hidden and non-blocking until hover */}
                ...
                {/* Arrow - subtle ghost by default */}
                <div className="w-10 h-10 flex items-center justify-center rounded-full bg-white/0 group-hover:bg-white/20 dark:group-hover:bg-black/20 transition-all duration-300 backdrop-blur-[2px]">
                    <span className="material-icons group-hover:translate-x-1 transition-transform text-2xl opacity-30 group-hover:opacity-100">arrow_forward</span>
                </div>
            </button>
        )}
